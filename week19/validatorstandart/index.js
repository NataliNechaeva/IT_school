class Validator {
    ...
    function validation () {
        ...
        let errors = [];
        if(validator.isEmail(document.getElementById('mail').value)==false) {
            { errors.push('Ваш адрес электронной почты введен неверно!');};
        }
        if (validator.isDomain(document.getElementById('domain').value)==false) {
		{ errors.push('Ваш домен введен неверно!');};
    }
    if (validator.isDate(document.getElementById('date').value)==true) {
		{ errors.push('Формат даты введен неверно!');};
    }
    if (validator.isPhone(document.getElementById('telephone').value)==false) {
